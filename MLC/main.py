...
parser.add_argument('--dataset', type=str, choices=['cifar10', 'cifar100', 'clothing1m', 'gtsrb', 'pneumonia'], default='cifar10')
...
def get_data(dataset, gold_fraction, corruption_prob, get_C):
    if dataset == 'cifar10' or dataset == 'cifar100':
        ...
        sys.path.append('GTSRB')

        from data_helper_gtsrb import prepare_data
        args.use_mwnet_loader = True # use exactly the same loader as in the mwnet paper
        logger.info('================= Use the same dataloader as in MW-Net =========================')
        return prepare_data(gold_fraction, corruption_prob, get_C, args)
    elif dataset == 'pneumonia':
        sys.path.append('Pneumonia')

        from data_helper_pneumonia import prepare_data
        args.use_mwnet_loader = True # use exactly the same loader as in the mwnet paper
        logger.info('================= Use the same dataloader as in MW-Net =========================')
        return prepare_data(gold_fraction, corruption_prob, get_C, args)
...
def build_models(dataset, num_classes):
    ...
    if dataset in ['cifar10', 'cifar100', 'gtsrb']:
        from CIFAR.resnet import resnet50, resnet18
        from CIFAR.vgg import vgg16, vgg11
        from CIFAR.mobilenet import mobilenet
        from CIFAR.convnet import convnet, deconvnet
        ...
        elif raw_model_name == "VGG16":
            model = vgg16(num_classes)
        elif raw_model_name == "VGG11":
            model = vgg11(num_classes)
        elif raw_model_name == "MobileNet":
            model = mobilenet(num_classes)
        elif raw_model_name == "ConvNet":
            model = convnet(num_classes)
        elif raw_model_name == "DeconvNet":
    ...
    elif dataset in ['pneumonia']:
        from Pneumonia.resnet import resnet50, resnet18
        from Pneumonia.vgg import vgg16, vgg11
        from Pneumonia.mobilenet import mobilenet
        from Pneumonia.convnet import convnet, deconvnet
...
def test(main_net, test_loader): # this could be eval or test
    ...
    noise_rate = args.corruption_level
    with open("./injection/" + args.dataset +"/mlc-" + main_net.name + "-" + "mislabel" + str(noise_rate) + "-" + str(identifier), "w") as w_file:
