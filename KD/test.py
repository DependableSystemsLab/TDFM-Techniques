...
def get_trained_model(model, model_type):
...
def main():
    ...
    teacher = get_trained_model(teacher, model_type)
    ...
    preds_test = teacher.predict(x_test)
    predictions = np.argmax(preds_test, axis=1)
    pred_list = predictions.tolist()

    with open("./injection/" + dataset_name + "/baseline-" + model_type + "-" + fault_str + "-" + str(identifier), "w") as w_file:
