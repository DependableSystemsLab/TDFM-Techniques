...
def perform_run(model_params, cluster_job, model_checkpoints, config=None):
    # Load data
    dataset_name = model_params.get_parameter("dataset_name")
    ...
    (x_train, y_train), (x_test, y_test), num_classes = get_dataset_by_type(dataset_type,
                                                                            ...
                                                                            model_params.get_parameter("final_fault"))
    ...
    if not model_params.get_parameter("data_augmentation"):
        conduct_simple_model_training(model, x_train, y_train, validation_data=val_data,
                                      model_parameters=model_params, callbacks=callbacks, shuffle=False,
                                      verbose=verbosity)
    else:
    ...
    if model_params.get_parameter("test_run"):
        ...
        fault_str = model_params.get_parameter("final_fault")

        with open("./injection/" + dataset_name + "/lr-" + model_params.get_parameter("model_name") + "-" + fault_str + "-" + str(identifier), "w") as w_file:
