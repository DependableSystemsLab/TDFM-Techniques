...
@click.option('--model_name', default='vgg', help='Backbone model',
              type=click.Choice(['resnet', 'ResNet18', 'ResNet50','vgg', 'VGG11', 'VGG16', 'VGG3', 'ConvNet', 'DeconvNet', 'MobileNet', 'simple_dense', 'densenet'], case_sensitive=False))
...
@click.option('--dataset_name', default='cifar100', help='Data set',
              type=click.Choice(['cifar100', 'imagenet', 'cifar10', 'mnist', 'fashion_mnist', 'gtsrb', 'pneumonia'], case_sensitive=False))
...
@click.option('--final_fault', default='golden', type=click.STRING)
def perform_single_experiment(model_name, loss_name, alpha, epochs, batch_size, seed, data_augmentation,
                              ...
                              lr_sched_multipler, reduce_on_plateau, test_run, load_ext_params, temp_scaling, final_fault):
    ...
    model_params.set_parameter("model_name", model_name)
    ...
    model_params.set_parameter("final_fault", final_fault)
    if load_ext_params:
